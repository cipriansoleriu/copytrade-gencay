...
- **Polling set** — `polling_addresses(shortlist, open_positions)` returns the shortlist **plus every trader whose paper position we still hold**, and the set allowed to open new ones (the shortlist only). Polling just the shortlist orphans positions: a trader who drops off at the daily refresh is never polled again, so their exit never produces a `CLOSED` signal and the paper position stays open forever with its margin locked. Entries follow current winners in; exits follow whoever we copied out.
- For each trader in the polling set:
  - `get_open_positions(addr)` → compare to `state/<addr>.json`'s prior `positions`.
  - `opened = curr - prev`, `closed = prev - curr` by coin:side.
  - For each opened **whose trader is in the shortlist**: append `{ts, trader, type:"NEW", **p}` to `signals.json`.
...
- **2026-08-25:** **Orphaned-position bug + paper book reset.** `job_positions.py` polled only the shortlist, so every daily rotation stranded the positions of dropped traders — never polled again, never a `CLOSED` signal, margin locked forever. By the time it was caught, **39 of 40 open positions were orphaned across 16 traders, $8,525.88 of margin dead**, cash down to $1,348 while sizing (5 % of cash) kept shrinking. The +9 % equity was almost entirely mark-to-market on positions nothing could ever close. Fixed with `polling_addresses()`: poll shortlist ∪ held, open only for the shortlist. Book reset to $10,000 (prior state archived as `*_pre_reset_2026-08-25.json`); `signals.json` and `state/` kept, with `last_processed_signal_index` pinned to the end so the engine does not replay history at today's prices.
- **2026-08-25:** GitHub Actions quota exhausted → **repo made public**, which makes standard runners free and unlimited. Jobs had been failing since 22 Aug 09:50 UTC with `runner_name` empty and `steps: []` — the signature of a job blocked before it is ever scheduled. **~3 days of silent downtime**; the report workflow could not alert because it was blocked too. Migration to n8n/Supabase/self-hosted runners was evaluated and proved unnecessary: repo visibility was a one-setting fix. Note the trap — driving Actions from an external scheduler does **not** avoid Actions billing; only public repos, self-hosted runners, or moving the compute off Actions do.
